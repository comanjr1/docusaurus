...
ThorProxy offers a comprehensive set of features designed for professional web automation and data collection.
...
response = requests.get(
    "https://tokopedia.com",
    proxies={"http": thorproxy_address}
)
...
for i in range(10):
    response = requests.get(
        url,
        proxies={"http": f"http://user-rotation:{password}@gateway.thorproxy.com:22225"}
    )
...
proxy = f"http://user-session-5min:{password}@gateway.thorproxy.com:22225"
```
...
proxy = f"http://user-sticky-30min:{password}@gateway.thorproxy.com:22225"
```
...
proxy = f"http://user-country-id:{password}@gateway.thorproxy.com:22225"
```
...
```python
# Only Jakarta IPs
proxy = f"http://user-city-jakarta:{password}@gateway.thorproxy.com:22225"

# Only Surabaya IPs  
proxy = f"http://user-city-surabaya:{password}@gateway.thorproxy.com:22225"
```
...
proxy = f"http://user-isp-indihome:{password}@gateway.thorproxy.com:22225"

# Only First Media connections
proxy = f"http://user-isp-firstmedia:{password}@gateway.thorproxy.com:22225"
```
...
proxies = {
    ...
    "https": "https://username:password@gateway.thorproxy.com:22225"
}
```
...
```python
proxies = {
    "http": "socks5://username:password@gateway.thorproxy.com:1080",
    "https": "socks5://username:password@gateway.thorproxy.com:1080"
}
```
...
ThorProxy is designed for legitimate business purposes:
- Market research
- Price monitoring
- Brand protection
- Ad verification
