...
# Introduction to LunaProxy

Welcome to **LunaProxy** - a powerful residential proxy network that provides authentic IP addresses from real home users around the world.

## What is LunaProxy?

LunaProxy is a residential proxy service that uses **real IP addresses** from home users' devices, provided by Internet Service Providers (ISP) such as:
...
LunaProxy operates on a **peer-to-peer network model** in partnership with applications that voluntarily share user bandwidth (with incentives). When you use their residential proxies, you're essentially "borrowing" an IP address from a real user in your target country.
...
Ready to start using LunaProxy? Check out our [Getting Started Guide](/docs/configuration/getting-started) to set up your first proxy connection.
