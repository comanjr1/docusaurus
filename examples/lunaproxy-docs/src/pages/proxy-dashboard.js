...
export default function ProxyDashboard() {
  ...
  useEffect(() => {
    ...
    const auth = localStorage.getItem('lunaproxy_auth');
    const user = localStorage.getItem('lunaproxy_user');
  ...
  const handleLogout = () => {
    localStorage.removeItem('lunaproxy_auth');
    localStorage.removeItem('lunaproxy_user');
    window.location.href = '/login';
  };
  ...
  return (
    <Layout
      ...
      description="LunaProxy Dashboard - View and manage your residential proxies">
